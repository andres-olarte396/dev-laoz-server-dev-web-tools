...
sudo apt-get update -y && sudo apt-get upgrade -y || { msg_error "Error al actualizar paquetes."; exit 1; }

# Instalamos Java (OpenJDK)
msg_success "Instalando Java (OpenJDK 17)..."
sudo apt-get install -y openjdk-17-jdk || { msg_error "Error al instalar Java."; exit 1; }
...
sudo apt-get install -y maven || { msg_error "Error al instalar Maven."; exit 1; }
...
wget -q https://repo.spring.io/release/org/springframework/boot/spring-boot-cli/$SPRING_CLI_VERSION/spring-boot-cli-$SPRING_CLI_VERSION-bin.tar.gz -O spring-boot-cli.tar.gz || { msg_error "Error al descargar Spring Boot CLI."; exit 1; }

# Extraemos y configuramos Spring Boot CLI
sudo tar -xzf spring-boot-cli.tar.gz -C /opt/
sudo ln -s /opt/spring-$SPRING_CLI_VERSION/bin/spring /usr/local/bin/spring || { msg_error "Error al configurar Spring Boot CLI."; exit 1; }
...
if [ $? -eq 0 ]; then
    ...
    msg_error "Error al instalar Spring Boot CLI."
    exit 1
fi
